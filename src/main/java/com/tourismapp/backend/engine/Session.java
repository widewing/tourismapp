...
public class Session {
	...
	private List<RouteSection> arrangeForDistrict(District district,Collection<Scenery> sceneries,Location startLocation,Date startTime){
	...
	private List<RouteSection> arrangeVisit(Location currentLoc,Date currentTime,Location visitLoc,int durationMinutes,float destinationPrice){
		LinkedList<RouteSection> routes = new LinkedList<RouteSection>();
	...
	private List<RouteSection> arrangeVisit(Location currentLoc,Date currentTime,Location visitLoc,Date visitUntil,float destinationPrice){
		LinkedList<RouteSection> routes = new LinkedList<RouteSection>();
		routes.addAll(arrangeInnerCityTransport(currentLoc,visitLoc,currentTime));
	...
	private List<RouteSection> arrangeHotel(Date currentTime,Location currentLoc){
		if(lastNightHotel!=null && lastNightHotel.getCoord().distance(currentLoc.getCoord())<20000)
			return arrangeVisit(currentLoc,currentTime,lastNightHotel,nextMorning(currentTime),lastNightHotel.getPrices().get(Hotel.RoomType.SINGLE.name()));
		...
		return arrangeVisit(currentLoc,currentTime,bestHotel,nextMorning(currentTime),bestHotel.getPrices().get(Hotel.RoomType.SINGLE.name()));
	}
	...
	private List<TransportSection> arrangeInterCityTransport(Location leaveLoc,District arriveLoc,Date leaveTime){
		Collection<Transport> availableTransports = transportation.getAvailableTransports(leaveTime, leaveLoc, arriveLoc,
	...
	private List<TransportSection> arrangeInnerCityTransport(Location leaveLoc,Location arriveLoc,Date leaveTime){
		Collection<Transport> availableTransports = transportation.getAvailableTransports(leaveTime, leaveLoc, arriveLoc,
					new Transport.Method[]{Transport.Method.Bus,Transport.Method.Taxi});
		return selectTransport(leaveLoc, arriveLoc, leaveTime, availableTransports);
	}
	private List<TransportSection> selectTransport(Location leaveLoc,Location arriveLoc,Date leaveTime,Collection<Transport> availableTransports){
	...
	private float calcTagsFitness(HashSet tags1,HashSet tags2){
		// TODO
		return 0;
	}
}
